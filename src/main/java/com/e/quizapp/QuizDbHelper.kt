...
import com.e.quizapp.QuizContract.QuestionsTable
...
class QuizDbHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    private fun fillQuestionsTable() {
        val q1 = Question("Does this app work?", "yes", "no", "maybe", 1)
        addQuestion(q1)
        val q2 = Question("B is correct", "yes", "no", "maybe", 2)
        addQuestion(q2)
        val q3 = Question("C is correct", "yes", "no", "maybe", 3)
        addQuestion(q3)
        val q4 = Question("A is correct again", "yes", "no", "maybe", 1)
        addQuestion(q4)
        val q5 = Question("B is correct again", "yes", "no", "maybe", 2)
        addQuestion(q5)
    }
